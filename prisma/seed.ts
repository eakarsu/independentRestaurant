...
async function main() {
  ...
  const waitlistNames = [
    ...
    { first: "Bob", last: "Brown" }, { first: "Alice", last: "White" },
    { first: "Tom", last: "Green" }, { first: "Maria", last: "Lopez" },
    { first: "James", last: "Chen" }, { first: "Sofia", last: "Patel" },
    { first: "Liam", last: "O'Brien" }, { first: "Emma", last: "Kim" },
    { first: "Noah", last: "Singh" }, { first: "Olivia", last: "Park" },
    { first: "Ethan", last: "Tanaka" }, { first: "Ava", last: "Nguyen" },
    { first: "Mason", last: "Rivera" }, { first: "Isabella", last: "Brooks" },
  ];

  for (let i = 0; i < 16; i++) {
    const wn = waitlistNames[i];
    await prisma.waitlist.create({
      data: {
        customerName: `${wn.first} ${wn.last}`,
        customerPhone: `(555) ${Math.floor(100 + Math.random() * 900)}-${Math.floor(1000 + Math.random() * 9000)}`,
        partySize: 2 + Math.floor(Math.random() * 6),
        estimatedWait: 10 + Math.floor(Math.random() * 45),
        status: i < 10 ? "WAITING" : (["SEATED", "NOTIFIED", "LEFT", "WAITING", "WAITING", "WAITING"] as const)[Math.floor(Math.random() * 6)],
      },
    });
  }
  ...
  const aiRecommendations = [
    ...
    { type: "schedule", title: "Cross-Train Hosts", description: "Train hosts on basic server tasks to handle overflow during peak hours." },
    { type: "marketing", title: "Instagram Food Photos", description: "Top-performing dishes lack social media presence. Weekly photo posts could increase walk-ins by 10%." },
    { type: "menu_price", title: "Increase Appetizer Portions", description: "Customer feedback indicates appetizer portions are small for the price. Consider 15% increase in portion." },
    { type: "inventory", title: "Pre-Order Lobster for Weekend", description: "Based on reservation data, 12+ lobster orders expected this weekend. Current stock insufficient." },
    { type: "schedule", title: "Stagger Break Times", description: "Lunch rush has a 15-minute gap in coverage. Stagger breaks to maintain service levels." },
    { type: "marketing", title: "Loyalty Tier Upgrade Push", description: "47 customers are within 100 points of Gold tier. Send targeted offer to drive visits." },
  ];
  ...
  const promotions = [
    ...
    {
      name: "Lunch Special",
      description: "10% off all lunch orders between 11am-2pm",
      code: "LUNCH10",
      type: "PERCENTAGE" as const,
      value: 10,
      minOrderAmount: 15,
    ...
    },
    {
      ...
      description: "Free dessert with any entree purchase",
      code: "VALENTINE",
      type: "FIXED_AMOUNT" as const,
      value: 12,
      minOrderAmount: 40,
    ...
    },
    {
      ...
      description: "20% off entrees before 6pm",
      code: null,
      type: "PERCENTAGE" as const,
      value: 20,
      minOrderAmount: 25,
    ...
    {
      name: "Takeout Tuesday",
      description: "$5 off all takeout orders on Tuesdays",
      code: "TAKEOUT5",
      type: "FIXED_AMOUNT" as const,
      value: 5,
      minOrderAmount: 20,
    ...
    {
      name: "Wine Wednesday",
      description: "Half price bottles of wine",
      code: null,
      type: "PERCENTAGE" as const,
      value: 50,
      minOrderAmount: null,
    ...
    },
    {
      ...
      description: "15% off for military and veterans",
      code: "MILITARY15",
      type: "PERCENTAGE" as const,
      value: 15,
      minOrderAmount: null,
    ...
    {
      name: "Spring Fling",
      description: "Free appetizer with $50+ orders",
      code: "SPRING",
      type: "FIXED_AMOUNT" as const,
      value: 14,
      minOrderAmount: 50,
  ...
  // Create some sample notifications (15+)
  const notifications = [
    {
      type: "ORDER_CONFIRMATION" as const,
      channel: "EMAIL" as const,
    ...
    },
    {
      ...
      subject: "Your Order #ORD-003 is Confirmed",
      message: "Hi James, your order has been confirmed and is being prepared.",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 5400000),
      deliveredAt: new Date(Date.now() - 5390000),
    },
    {
      type: "RESERVATION_CONFIRMATION" as const,
      channel: "EMAIL" as const,
      recipient: "patricia.taylor@email.com",
      subject: "Reservation Confirmed for Tonight",
      message: "Your reservation for 4 at 7:30 PM has been confirmed. Table 8 is reserved for you.",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 14400000),
      deliveredAt: new Date(Date.now() - 14390000),
    },
    {
      type: "FEEDBACK_REQUEST" as const,
      channel: "EMAIL" as const,
      recipient: "robert.anderson@email.com",
      subject: "How was your experience?",
      message: "Hi Robert, we hope you enjoyed your meal. Please take a moment to share your feedback.",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 172800000),
      deliveredAt: new Date(Date.now() - 172790000),
    },
    {
      type: "ORDER_READY" as const,
      channel: "SMS" as const,
      recipient: "+14155550789",
      subject: null,
      message: "Your takeout order #ORD-004 is ready! Pick up at the counter.",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 900000),
      deliveredAt: new Date(Date.now() - 890000),
    },
    {
      type: "LOYALTY_UPDATE" as const,
      channel: "EMAIL" as const,
      ...
      message: "Congratulations David! You've been upgraded to Gold tier. Enjoy exclusive benefits.",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 259200000),
      deliveredAt: new Date(Date.now() - 259190000),
    },
    {
      type: "RESERVATION_REMINDER" as const,
      channel: "SMS" as const,
      ...
      subject: null,
      message: "Don't forget! Your reservation is today at 8:00 PM for 6 guests.",
      status: "SENT" as const,
    ...
    },
    {
      ...
      subject: null,
      message: "Order confirmed! Estimated delivery in 45 minutes.",
    ...
    },
    {
      type: "FEEDBACK_REQUEST" as const,
      channel: "EMAIL" as const,
      recipient: "susan.allen@email.com",
      subject: "We'd love your feedback!",
      message: "Hi Susan, thank you for dining with us last night. Your opinion matters!",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 345600000),
      deliveredAt: new Date(Date.now() - 345590000),
    },
    {
      type: "LOYALTY_UPDATE" as const,
      channel: "EMAIL" as const,
      recipient: "karen.adams@email.com",
      subject: "Exclusive VIP Offer Inside",
      message: "As a valued VIP member, enjoy a complimentary appetizer on your next visit.",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 432000000),
      deliveredAt: new Date(Date.now() - 431990000),
    },
    {
      type: "RESERVATION_CONFIRMATION" as const,
      channel: "SMS" as const,
      ...
      subject: null,
      message: "Reservation confirmed! Party of 2 at 6:30 PM on Friday. See you then!",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 21600000),
      deliveredAt: new Date(Date.now() - 21590000),
    },
    {
      type: "ORDER_READY" as const,
      channel: "EMAIL" as const,
      recipient: "thomas.wright@email.com",
      subject: "Your Order is Ready!",
      message: "Your order #ORD-005 is ready for pickup. Thank you for choosing us!",
      status: "DELIVERED" as const,
      sentAt: new Date(Date.now() - 2700000),
      deliveredAt: new Date(Date.now() - 2690000),
    },
    {
      type: "FEEDBACK_REQUEST" as const,
      channel: "SMS" as const,
      recipient: "+14155554567",
      subject: null,
      message: "Rate your experience! Reply 1-5 stars. Thank you for dining with us.",
      status: "SENT" as const,
  ...
  const recipeMenuItems = await prisma.menuItem.findMany({ take: 15 });
  ...
  const additionalRecipes = [
    {
      ...
      difficulty: "EASY" as const,
      instructions: [
        { step: 1, text: "Prepare calamari by cutting into rings and patting dry." },
        { step: 2, text: "Season flour mixture with salt, pepper, and paprika." },
        { step: 3, text: "Dredge calamari in flour, shaking off excess." },
        { step: 4, text: "Fry at 375°F for 2-3 minutes until golden." },
        { step: 5, text: "Drain on paper towels and serve with marinara and aioli." },
      ],
      notes: "Don't overcook - calamari becomes rubbery. Serve immediately.",
      allergenNotes: "Contains shellfish and wheat.",
    },
    {
      prepTime: 20,
      cookTime: 45,
      ...
      difficulty: "HARD" as const,
      instructions: [
        { step: 1, text: "Season lamb chops with rosemary, garlic, salt, and pepper." },
        { step: 2, text: "Let marinate for at least 30 minutes at room temperature." },
        { step: 3, text: "Sear in a hot skillet for 3 minutes per side." },
        { step: 4, text: "Transfer to oven at 400°F for 5-7 minutes for medium-rare." },
        { step: 5, text: "Prepare mint pesto by blending fresh mint, pine nuts, garlic, and olive oil." },
        { step: 6, text: "Rest 5 minutes. Plate with mint pesto drizzle and roasted vegetables." },
      ],
      notes: "New Zealand lamb preferred. Internal temp: 135°F for medium-rare.",
      allergenNotes: "Contains tree nuts in pesto.",
    },
    {
      prepTime: 15,
      cookTime: 30,
      ...
      difficulty: "MEDIUM" as const,
      instructions: [
        { step: 1, text: "Cook fettuccine until al dente. Reserve 1 cup pasta water." },
        { step: 2, text: "Melt butter in large saucepan over medium heat." },
        { step: 3, text: "Add heavy cream and bring to a gentle simmer." },
        { step: 4, text: "Gradually stir in parmesan cheese until smooth." },
        { step: 5, text: "Toss pasta in sauce, adding pasta water for desired consistency." },
        { step: 6, text: "Season with salt, white pepper, and nutmeg. Garnish with parsley." },
      ],
      notes: "Use freshly grated Parmigiano-Reggiano for best results.",
      allergenNotes: "Contains dairy and wheat.",
    },
    {
      prepTime: 5,
      cookTime: 20,
      ...
      difficulty: "EASY" as const,
      instructions: [
        { step: 1, text: "Slice onions into thin rounds and separate into rings." },
        { step: 2, text: "Prepare beer batter with flour, baking powder, and cold beer." },
        { step: 3, text: "Dip onion rings in batter, letting excess drip off." },
        { step: 4, text: "Fry at 365°F for 3-4 minutes until golden brown." },
        { step: 5, text: "Drain on wire rack. Season immediately with salt." },
      ],
      notes: "Use a light lager for the batter. Keep oil temperature consistent.",
      allergenNotes: "Contains wheat and gluten.",
    },
    {
      prepTime: 30,
      cookTime: 15,
      ...
      difficulty: "HARD" as const,
      instructions: [
        { step: 1, text: "Select sashimi-grade ahi tuna and dice into small cubes." },
        { step: 2, text: "Prepare marinade with soy sauce, sesame oil, rice vinegar, and ginger." },
        { step: 3, text: "Toss tuna gently in marinade. Let sit 5 minutes." },
        { step: 4, text: "Dice avocado and mix with lime juice." },
        { step: 5, text: "Layer avocado on plate, top with seasoned tuna." },
        { step: 6, text: "Garnish with sesame seeds, microgreens, and wonton chips." },
      ],
      notes: "Must use sashimi-grade fish. Keep tuna very cold until plating.",
      allergenNotes: "Contains fish, soy, sesame, and wheat.",
    },
    {
      prepTime: 10,
      cookTime: 20,
      ...
      instructions: [
        ...
        { step: 2, text: "Trim asparagus ends and arrange on baking sheet." },
        { step: 3, text: "Drizzle with olive oil, season with salt and pepper." },
        { step: 4, text: "Roast for 12-15 minutes until tender-crisp." },
        { step: 5, text: "Squeeze fresh lemon juice over top before serving." },
      ],
      notes: "Medium thickness spears work best. Don't overcook.",
      allergenNotes: "No common allergens.",
    },
    {
      prepTime: 25,
      cookTime: 60,
      ...
      difficulty: "HARD" as const,
      instructions: [
        { step: 1, text: "Prepare graham cracker crust and press into springform pan." },
        { step: 2, text: "Bake crust at 325°F for 10 minutes." },
        { step: 3, text: "Beat cream cheese until smooth. Add sugar and vanilla." },
        { step: 4, text: "Add eggs one at a time, mixing gently after each." },
        { step: 5, text: "Pour filling into crust. Bake 50-55 minutes until set but slightly jiggly." },
        { step: 6, text: "Cool slowly in oven with door cracked to prevent cracking." },
        { step: 7, text: "Refrigerate overnight. Top with berry compote before serving." },
      ],
      notes: "Room temperature ingredients are key. Don't overmix after adding eggs.",
      allergenNotes: "Contains dairy, eggs, and wheat.",
    },
    {
      prepTime: 15,
      ...
      difficulty: "MEDIUM" as const,
      instructions: [
        { step: 1, text: "Brine chicken breast for 30 minutes in salt water." },
        { step: 2, text: "Bread chicken: flour, egg wash, then panko breadcrumbs." },
        { step: 3, text: "Pan-fry until golden on both sides." },
        { step: 4, text: "Top with marinara and fresh mozzarella." },
        { step: 5, text: "Broil until cheese is melted and bubbly." },
        { step: 6, text: "Serve over spaghetti with extra marinara and basil." },
      ],
      notes: "Pound chicken to even thickness for uniform cooking.",
      allergenNotes: "Contains wheat, dairy, and eggs.",
    },
    {
      prepTime: 10,
      cookTime: 15,
      ...
      difficulty: "EASY" as const,
      instructions: [
        { step: 1, text: "Combine heavy cream, sugar, and vanilla in saucepan." },
        { step: 2, text: "Heat until sugar dissolves. Do not boil." },
        { step: 3, text: "Whisk in bloomed gelatin until dissolved." },
        { step: 4, text: "Divide among ramekins and refrigerate 4+ hours." },
        { step: 5, text: "Sprinkle sugar on top and torch until caramelized." },
      ],
      notes: "Torch just before serving for the best crackle.",
      allergenNotes: "Contains dairy and eggs.",
    },
    {
      prepTime: 20,
      cookTime: 25,
      ...
      difficulty: "MEDIUM" as const,
      instructions: [
        { step: 1, text: "Season sea bass with miso glaze (white miso, mirin, sugar, sake)." },
        { step: 2, text: "Marinate for at least 2 hours, ideally overnight." },
        { step: 3, text: "Bring to room temperature before cooking." },
        { step: 4, text: "Broil on high for 8-10 minutes until caramelized." },
        { step: 5, text: "Serve on bed of bok choy with sesame drizzle." },
      ],
      notes: "Watch carefully under broiler as miso glaze can burn quickly.",
      allergenNotes: "Contains fish and soy.",
    },
  ];

  const allRecipes = [...recipeData, ...additionalRecipes];
  for (let i = 0; i < Math.min(allRecipes.length, recipeMenuItems.length); i++) {
